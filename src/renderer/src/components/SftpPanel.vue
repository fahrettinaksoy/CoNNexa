...
async function remove(entry: SftpEntry): Promise<void> {
  const result = await window.connexa.sftp.delete(
    props.sessionId,
    entry.path,
    entry.type === 'dir'
  )
    ...
    <v-dialog v-model="mkdirDialog" max-width="360">
      <v-card :title="t('sftp.newFolder')">
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" variant="flat" @click="confirmMkdir">{{ t('common.save') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="renameDialog" max-width="360">
      <v-card :title="t('sftp.rename')">
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" variant="flat" @click="confirmRename">{{ t('common.save') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
