...
export interface ResponsiveLayout {
  ...
  /** Platform bayrakları (electron, mac, win, touch, …). */
  platform: Display['platform']
  /** `<v-app>` köküne basılacak platform sınıfları (ör. "platform-electron platform-mac"). */
...
export function useResponsive(): ResponsiveLayout {
  ...
  const platformClasses = computed(() => {
    ...
    if (p.electron) classes.push('platform-electron')
