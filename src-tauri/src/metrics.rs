...
pub async fn snapshot(handle: &SshHandle) -> HostMetrics {
    let out = match ssh::exec(handle, CMD).await {
        ...
        Err(e) => return HostMetrics { ok: false, error: Some(e), ..Default::default() },
    };
    parse(&out)
}
...
fn parse(out: &str) -> HostMetrics {
    let mut m = HostMetrics { ok: true, ..Default::default() };
    ...
    for line in disk.lines().skip(1) {
        ...
        if mount != "/" && (mount.starts_with("/dev") || mount.starts_with("/proc") || mount.starts_with("/sys") || mount.starts_with("/run")) {
            continue;
        }
        ...
        disks.push(DiskUsage { mount: mount.to_string(), size_bytes: size, used_bytes: used, use_percent: pct });
    }
    if !disks.is_empty() {
        m.disks = Some(disks);
    }
    ...
    for line in proc.lines().skip(1) {
        ...
        procs.push(ProcessInfo { pid, cpu, mem, command });
    }
    if !procs.is_empty() {
